...
CeptonSubscriber::CeptonSubscriber() : Node("cepton_subscriber") {
  ...
  bool subscribeCeptonPoints = true;
  bool subscribeCeptonInfo = true;
  ...
  declare_parameter("export_to_csv", exportToCsv_);
  ...
  if (pExportCsv.get_type() != rclcpp::ParameterType::PARAMETER_NOT_SET)
    exportToCsv_ = pExportCsv.as_bool();

  // Subscribe to PointCloud2
  if (subscribePcl2)
    pointsSubscriber = create_subscription<PointCloud2>(
        "cepton_pcl2", 10,
        bind(&CeptonSubscriber::recvPoints, this, placeholders::_1));
  ...
  if (subscribeCeptonPoints)
    ceptonPointsSubscriber =
        create_subscription<cepton_messages::msg::CeptonPointData>(
            "cepton_points", 50,
            bind(&CeptonSubscriber::recvCepPoints, this, placeholders::_1));
  ...
  if (subscribeCeptonInfo)
    infoSubscriber =
        create_subscription<cepton_messages::msg::CeptonSensorInfo>(
            "cepton_info", 10,
            bind(&CeptonSubscriber::recvInfo, this, placeholders::_1));
}

void CeptonSubscriber::recvInfo(
    const cepton_messages::msg::CeptonSensorInfo::SharedPtr info) {
  printf("Serial number: %d\tModel: %s\tHandle: %u\n", info->serial_number,
         info->model_name.c_str(), (uint32_t)info->handle);
}

void CeptonSubscriber::recvPoints(
...
void exportCsv(cepton_messages::msg::CeptonPointData::SharedPtr pPoints) {
  ...
  ofstream s(f);
  s << "timestamp,x,y,z,c,r,flag" << endl;
